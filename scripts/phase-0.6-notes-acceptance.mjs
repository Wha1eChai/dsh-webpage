...
const notesPath = '/apps/wha1echai.notes'
...
try {
  ...
  await page.locator('[data-app-id="wha1echai.notes"]').first().waitFor({ state: 'visible', timeout: 15_000 })
  const ids = await page.locator('[data-app-id]').evaluateAll(nodes => [...new Set(nodes.map(node => node.getAttribute('data-app-id')))])
  assert(ids.includes('wha1echai.notes') && ids.includes('wha1echai.usage') && ids.includes('wha1echai.webpage'), `launcher ids: ${JSON.stringify(ids)}`)
  ...
  await composer.fill('请使用 open_app 工具打开鲸鱼笔记，app_id 是 wha1echai.notes，直接调用。')
  await page.keyboard.press('Enter')

  const card = page.locator('[data-open-app="wha1echai.notes"]')
