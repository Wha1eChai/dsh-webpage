...
async function assertManifest() {
  ...
  for (const dir of ['reference-app', 'reference-extension', 'reference-pack']) {
...
async function assertBuilt() {
  ...
  assert(existsSync(nodePath) && existsSync(invariantPath) && existsSync(clientPath), 'core Node/invariant/client artifacts are missing; run pnpm build first')
  ...
  for (const path of [nodePath, invariantPath, clientPath]) {
